...
Mgly.ok = {
	typedArrays : (typeof Uint8Array == "function") ? true : false
};
...
jQuery.extend(Mgly.CodeifyText.prototype, {
	...
	_diff_ctx: function(lines) {
		var ctx = {
				i: 0, 
				// codes are ccessed as an array, not necessarily an {} 
				codes: Mgly.ok.typedArrays ? new Uint16Array(lines.length) : [], 
				lines: lines
			};
...
Mgly.diff = function(lhs, rhs, options) {
	...
	// localize
	var useTyped = Mgly.ok.typedArrays;
	...
	// need to get the length later on...
	var lines_l = this.codeify.getCodes('lhs');
	var lines_r = this.codeify.getCodes('rhs');
	
	var lhs_ctx = {
		codes: lines_l,
		modified: useTyped ? new Uint8Array(lines_l.length) : {} // {}
	};
	var rhs_ctx = {
		codes: lines_r,
		modified: useTyped ? new Uint8Array(lines_r.length) : {} // {}
	};

	// Need to double total number of lines to cover both having nothing in common.
	var doublev = parseInt( lines_l.length + lines_r.length + 1 ) * 2;
	var vector_d = useTyped ? new Uint16Array(doublev) : [];
	var vector_u = useTyped ? new Uint16Array(doublev) : [];
